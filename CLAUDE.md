...
3. **Check `packages/private/web-core/src/components/` and `apps/website/src/components/common/` before building anything.** The shared, app-agnostic half lives in the package; the 14 components that depend on the dashboard's session or guild data stayed in the app. `TextField`, `TextAreaField`, `RawJsonField`, `SnowflakeInput`, `EmojiInput`, `ChannelSelect`, `RoleSelect`, `ForumTagSelect`, `ConfirmModal`, `EmptyState`, `Skeleton`, `Tooltip`, `SearchBar`, `FormActions` and more already exist. Search first; do not reinvent.

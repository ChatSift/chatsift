	...
	return (
		<div className="flex flex-col gap-6">
			<div className="flex flex-col gap-3">
				...
				/>
				<UserBadge id={report.targetId} size="lg" storedTag={report.targetTag} user={report.target} />
			</div>
			...
			<div className="flex flex-col gap-4 rounded-lg border border-on-secondary bg-card p-4 dark:border-on-secondary-dark dark:bg-card-dark">
				<Field label="What was reported">
					{report.messageId ? (
						...
						// An account-level report has no content to quote. The caveat is not decoration: a reporter
						// complaining about somebody's *profile* is the common reason to file one of these, and the bot
						// can see none of it -- a bio, pronouns and display-name history are all absent from
						// `GET /users/{id}`. Staff who don't know that will read this panel as "there was nothing to
			...
			<div className="flex flex-col gap-3 rounded-lg border border-on-secondary bg-card p-4 dark:border-on-secondary-dark dark:bg-card-dark">
				...
				<div className="flex flex-col gap-3">
					{reporters.map((entry) => (
						<div className="flex flex-col gap-1" key={entry.reporterId}>
							<UserBadge id={entry.reporterId} storedTag={entry.reporterTag} user={entry.reporter} />

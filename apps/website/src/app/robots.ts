 ...
 *
 * `/ama-answers/*` is deliberately *not* listed, despite being the other surface we don't want indexed. Those
 * pages carry `robots: { index: false, follow: false }` instead, and the two mechanisms don't stack: a
 * crawler has to be allowed to fetch a page before it can see that page's `noindex`. Disallowing them would
 * suppress the fetch and leave a publicly-linked share URL eligible to show up as a bare, snippet-less
 * result -- the exact outcome the `noindex` is there to prevent. Allowing the fetch is what makes it stick.
 */

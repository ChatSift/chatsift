...
- **ModMail** — staff-thread relay for user inquiries. `ChatSift/ModMail` is DM-based; the in-repo rebuild ([01-architecture.md §6a](01-architecture.md#6a-modmail-bot-subsystem-servicesmodmail-bot-m5), shipped, redesigned 2026-07-22) replaces the DM origin with an in-server private-thread ticket flow — the mod-side experience is unchanged. Only the legacy data migration + cutover are still outstanding, see [06-modmail-port.md](06-modmail-port.md).
...
3. **ModMail is ported** to the new monorepo, including a **real data migration** of threads/messages/snippets/blocks from `ChatSift/ModMail`. The port itself (schema/API/dashboard/bot, see [01-architecture.md §6a](01-architecture.md#6a-modmail-bot-subsystem-servicesmodmail-bot-m5)) shipped 2026-07-30; the data migration + cutover are what's left — see [06-modmail-port.md](06-modmail-port.md).
...
| M5 — ModMail rebuild + migrate                                         | [01-architecture.md §6a](01-architecture.md#6a-modmail-bot-subsystem-servicesmodmail-bot-m5) (shipped feature work); [06-modmail-port.md](06-modmail-port.md) (remaining scope) | Freeze 2026-08-24 → migration 2026-08-26 (both 15:00Z), announced to owners                 | Feature work done (2026-07-30); pre-cutover comms in flight (#313); legacy data migration + cutover from `ChatSift/ModMail` not started |
| M6 — AMA question management (prepared answers, triage, tags, merging) | [07-ama-question-management.md](07-ama-question-management.md)                                                                                                                  | None announced — feature request, not a public commitment                                   | Implemented 2026-08-06; live verification against a test guild not yet run                                                              |

M1–M3's per-milestone spec docs (`02-foundation.md`, `03-dashboard-config.md`, `04-ama-complete.md`) were removed once each milestone shipped — durable architecture knowledge from them now lives in [01-architecture.md](01-architecture.md) and [workflow.md](../workflow.md); planning detail is in git history if ever needed. M4's dates come from a public Discord announcement, not a capacity estimate. M5 was fundamentally redesigned on 2026-07-22 (see [06-modmail-port.md](06-modmail-port.md) for why); the feature work that redesign scoped shipped 2026-07-30 and now lives in [01-architecture.md §6a](01-architecture.md#6a-modmail-bot-subsystem-servicesmodmail-bot-m5) the same way M1–M3's did, following the same "shipped work moves into the architecture doc" pattern.

...
- `components/submitQuestion.ts` — user clicks "Submit a question" on the prompt message → modal (text + optional uploads, gated by `allowedQuestionUploads`) → inserts `AMAQuestion` → routes into the queue or straight to answers per `reviewEnabled`/`preparedAnswersEnabled`. Rejects submission once `ended`.

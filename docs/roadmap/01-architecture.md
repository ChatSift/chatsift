...
- **Umbrella questions (#366)** - `POST .../questions` (`createQuestion.ts`) is the only way to author a question that did not arrive through the bot's submit modal. Staff write the wording they want to answer, real duplicates get merged into it, and the asker tally lands on their phrasing instead of whichever submission happened to be picked as the original. It is created `APPROVED` with no `queueMessageId`, whatever the session's review/prepared-answer settings say: `PENDING_REVIEW` would ask staff to review their own question (and there is no queue message for the Discord buttons to sit on), `ASKED` would publish it before anything could be merged in, and `APPROVED` is already a `MERGE_TARGET_STATE` that the dashboard's existing Send action publishes. `authorId` is the dashboard user who wrote it, and the row carries `umbrella` alongside `anonymous` (both true at creation).
  ...
  **The wording stays editable too**, through `updateQuestion.ts`'s `content` mode and an Edit button on the detail panel's question block. This reverses the #366 call to reject "making an existing question's content editable" - that call was about _submitted_ questions and still holds for them: the route refuses `content` on any row without `umbrella`, because a submission's wording is what its asker actually typed and rewriting it would publish someone else's words under their name. An umbrella question has no such owner; the staffer who wrote it is the one editing it. The Discord-first strictness is the same as every other edit branch, with one difference: both surfaces render the content, so it re-renders whichever message `resolveCurrentQueueMessage` returns rather than only the answers one. In practice that is only ever the answers message, since an umbrella question is created with no queue message at all. The dashboard confirms before saving a reword that is already public - which is _sent_, not _answered_: the question embed goes up the moment the question is sent, so unlike the answer editor's own confirm this keys off the state, not off `answerContent`.

- **Both shipped behind the `ama-qol` experiment and have since graduated** - #366 was the first feature in the repo to actually use the `experiments` machinery P0 of the AutoModerator port built (see [11-automoderator-port.md](11-automoderator-port.md#feature-gating)). At 100% rollout the gate was stripped: no `isExperimentEnabled` call sites remain, and `@chatsift/core`'s `experimentNames.ts` went with the last name in it. The conventions it established survive as conventions - a gate's name belongs in a shared `@chatsift/core` constant so the API, the bot and the dashboard check the same string (that note now lives on `isExperimentEnabled` itself), and a gate covers **write paths and the UI that reaches them, never rendering**, so that switching a kill switch off cannot retroactively name someone who was published anonymously. The machinery is all still wired: the `/v3/experiments` CRUD, the `/admin` console, `loadExperiments()` at boot in `services/api`/`services/ama-bot`/`services/automoderator-bot`, and `MeGuild.experiments`. Deleting the `ama-qol` row itself is an operator step in `/admin`, not a code change.
- **How the dashboard learns a gate's state** (no current consumer, kept for the next one): `enabledExperimentsFor(guildId)` on each `MeGuild`, i.e. `/v3/auth/me`, read through the `useExperiment` hook. Deliberately _not_ `GET /v3/guilds/:guildId`, the obvious home for a per-guild flag: that route is hard manager-only and `useGuildInfo` skips it entirely for an AMA guest, who would then see every gated AMA control hidden no matter the gate - and guests act on the queue. `me.guilds` reaches them because the API synthesizes an entry for a guest who isn't a Discord member at all. The cost is staleness: the list rides the 5-minute `Me` cache on top of the snapshot's own 60s refresh, so the UI lags a gate change by longer than enforcement does. It errs towards hiding an available control, which is the harmless direction, and adding the field to the cache's bin-rw recipe is safe because `RedisStore`'s `decodeOrEvict` evicts a stale entry on schema mismatch rather than throwing.
...
**Inbound interest** (`modmail_instance_interest`): the one self-serve half of the arrangement. The ModMail dashboard root (`apps/website/src/app/dashboard/[id]/modmail/_components/CustomInstanceUpsell.tsx`) offers a guild manager a one-way "we're interested" button, and `/admin` lists what it collected (`routes/modmail/instanceInterest/`, global-admin only). One row per guild, keyed on `guild_id` because an instance is a per-guild arrangement, recording whoever pressed first plus a snapshot of the guild's name at that moment - the user half is resolved live instead, since `GET /users/{id}` keeps working forever while a guild's name can only be read through a bot that is still in it. There is deliberately no delete route: a lead that turns out to be stale costs a conversation, a withdrawal that reaches nobody costs a partner. Everything past the button is still the hand-run runbook in [workflow.md](../workflow.md#custom-modmail-instances-216).

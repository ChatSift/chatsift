...
- [x] **`/ama create`** — deliberately **not** a creation flow. Implemented as `commands/ama.ts`'s `create` subcommand: an ephemeral reply linking to the dashboard's AMA-create screen at `${FRONTEND_URL}/dashboard/:guildId/ama/amas/new?token=...`. **Unverified end-to-end** — build+lint are green but this hasn't been click-tested against a live guild yet (see Verification).
  - **Grant-token auth, implemented (#194, `feat/sec-fixes`):** the deferred idea below shipped. `createGrantToken()` (`packages/private/backend-core/src/lib/grantToken.ts`) mints a short-lived (15m), single-use JWT scoped to `{ sub: userId, guildId, grant: 'ama:create' }` — both facts already proven by the user having been able to run the slash command in that guild — and embeds it in the same dashboard URL rather than routing to a separate page. See [01-architecture.md §4a](01-architecture.md#4a-grant-token-auth-one-time-scoped-194) for the full mechanism (how the API/frontend/proxy all recognize and scope it) and the security reasoning.
...
**Grant-token flow (#194) specifically**, in a browser with no dashboard session (private window, or logged-out): run `/ama create`, click the link, confirm the create form loads without an OAuth redirect and the dashboard chrome (breadcrumbs, guild nav tabs) renders as non-interactive; submit and confirm the in-place success state (no redirect to the AMA list, which isn't grant-accessible); reload/resubmit the same link afterward and confirm it's rejected as already-used. Separately, with a real session logged in, confirm opening the link in another tab doesn't disturb the logged-in tab (no forced re-login, no logout) — see [01-architecture.md §4a](01-architecture.md#4a-grant-token-auth-one-time-scoped-194) for what's actually being isolated.

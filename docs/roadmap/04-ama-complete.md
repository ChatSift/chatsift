...
- [ ] **Flagged/reported queue** — same: confirm or add `flaggedApprove`/`flaggedDeny` (or a "flag" action from the mod queue routing into it) mirroring the existing pattern. A question in the flagged queue should have a defined resolution path (approve → back into the normal flow at the right stage, or deny → terminal).

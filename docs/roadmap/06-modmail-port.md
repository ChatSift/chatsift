...
2. User clicks the button → bot prompts them to pick a **category**, ephemerally (only the ticket opener sees it) — no private thread exists yet at this point. A panel with no categories configured skips straight to step 4.
3. Category selection → bot opens a **private thread** for them (in the panel's designated parent channel) and tells them, ephemerally, to describe what they need help with there.
4. Once the user's opening message actually arrives in that private thread, the bot opens the corresponding **forum thread** for mods (in the designated mod forum channel) and relays that first message into it.

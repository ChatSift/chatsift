...
- **Duplicates are a separate, more opinionated feature from tags**: mark question A as a duplicate of question B → A is deleted, B gains A's asker → B renders as "Asked by X, Y, Z [...and N more]" → same flow exists in Discord (button + modal search + select) as well as the dashboard. _(Shipped narrower than written: Discord shows only a count, "Also asked by — N other people", never the names — see #326 and [01-architecture.md §5](01-architecture.md#5-data-model-reference-6-models). #328 later widened which states can absorb a duplicate.)_

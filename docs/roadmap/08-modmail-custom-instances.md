...
## Status: P1 shipped 2026-07-29 (bot-side registry + ownership gating); P2 (instance-aware API) implemented 2026-07-29, not yet manually verified against a real custom instance

**Picking this up in a new session? Read this before touching P3:**
...
- `discordApplication.ts`'s `getModmailApplicationId` now takes `guildId`, keyed internally by `'public'` or the owning instance's id — a partner's bot application has its own id, distinct from the public one. Same file gained `getInstanceBranding(instance)`, returning `{ label, iconUrl }` — `label` is just the registry row's `label`, `iconUrl` comes from `applications.getCurrent()` on the instance's own token and is cached in redis (`modmail:instance-branding:<id>`, 1h TTL) since an icon can change over the life of a process, unlike the application id.
...
### P2 — Instance-aware API — implemented 2026-07-29, not yet manually verified

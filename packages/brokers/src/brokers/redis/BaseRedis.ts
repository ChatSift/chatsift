...
import type { BaseBrokerOptions, IBaseBroker, ToEventMap } from '../Broker';
import { DefaultBrokerOptions } from '../Broker';
	...
	implements IBaseBroker<TEvents>
{
	...
	public constructor(options: RedisBrokerOptions) {
		...
		options.redisClient.defineCommand('xcleangroup', {
			numberOfKeys: 1,
			lua: readFileSync(
				resolve(fileURLToPath(import.meta.url), '..', '..', '..', '..', 'scripts', 'xcleangroup.lua'),
				'utf8',
			),
		});

...
CREATE TABLE ama_questions (
  ...
  -- Written from the dashboard for real duplicates to be merged under (#366), rather than submitted
  -- through the bot's modal like every other row here. `author_id` is the moderator who typed it, kept
  -- for the audit trail and shown on mod-facing surfaces as "written by" -- but they never asked the
  -- question, so the audience is never shown an author for one of these (the row is created with
  -- `anonymous` already true, and every rendering path treats umbrella as forcing it regardless).
  -- That difference is also why the merged-asker line has to know: an umbrella question's askers are
  -- exactly the merges, while a plain anonymous question's are the merges plus the author nobody sees.
  umbrella                 BOOLEAN NOT NULL DEFAULT false,
  -- Whether the merged-asker line ("Asked by N people") renders at all (#366 PM feedback). Only the
  -- umbrella create/edit UI ever sets it false -- for every other question the count is the whole point
  -- of a merge, so the API refuses to clear it on a non-umbrella row rather than a CHECK constraint
  -- doing it, which keeps the refusal explainable to whoever tripped it.
  show_asker_count         BOOLEAN NOT NULL DEFAULT true,
  created_at               TIMESTAMPTZ NOT NULL DEFAULT now(),
...
-- Context messages a report carries *beyond* the one on `automoderator_reports` itself (P3b, DM reporting).
--
-- The parent's `message_id`/`message_content`/`message_image_url` stay the subject message: the one that
-- dedupes, the one the card leads with, the one that caused the report. These are the rest of the draft, and
-- they are a different kind of row -- a DM draft can legitimately include the *reporter's own* replies, since a
-- baited message reads very differently on its own. Hence `author_id`/`author_tag`, which the parent never
-- needs (there the author is always the target).
--
-- Nothing but a DM report writes these today. A GUILD report has exactly zero rows here, which is why this is
-- additive rather than a table every report's first message moved into.
CREATE TABLE automoderator_report_messages (
  ...
  -- 1-based, because position 0 is the parent row's own snapshot. Order is the reporter's, taken from the
  -- order they added messages to the draft -- not chronological, and deliberately not re-sorted: which message
  -- they lead with is part of what they are saying.
  position   SMALLINT NOT NULL,
  message_id TEXT NOT NULL,
  -- The DM channel. Stored for completeness and never rendered as a link -- `origin = 'DM'` means staff could
  -- not open it (see `automoderator_report_origin`).
  channel_id TEXT NOT NULL,
  author_id  TEXT NOT NULL,
  ...
  -- No `created_at`: the report's own is when it was filed, and each message's own time is already in its
  -- snowflake. A third timestamp would only be the moment the draft was flushed, which means nothing to staff.

  CONSTRAINT automoderator_report_messages_report_id_position_key UNIQUE (report_id, position),
  -- The same message twice in one report is a mis-click, not evidence.
...
-- The DM-reporting install prompt (P3b). Staff post one of these into a channel members read, because the
-- realistic path to DM reporting is five steps -- bad DM, find the prompt, install the user app, back to the
-- DM, use the menu -- and the install link has to be findable rather than buried in a support thread.
--
-- Deliberately many-per-guild and shaped exactly like `ticket_panels`: same `channel_id`/`message_id` pair so
-- an edit rewrites the message in place, same `*_json_data` snapshot of what was posted. The one structural
-- difference from a ticket panel is that the button is a **link** to Discord's user-install URL rather than a
-- `custom_id`, so nothing ever routes an interaction back here -- the message is inert once posted.
...
-- Roles whose holders every filter ignores (P5, feature 10). Legacy keyed this on `role_id` alone -- globally
-- unique across every guild, the same shape `log_ignores` had and the same latent cross-guild delete. Guild-
-- scoped here.
--
-- Distinct from Discord's own per-rule exempt roles, which are the right place to stop a *match* happening at
-- all. This is the layer below: the match still happened, we still log it, and the response is skipped. That
-- split is deliberate, because "the filter never saw it" and "the filter saw it and let them off" are the two
-- things a moderator asking why nothing happened needs told apart.
...
-- What a banword hit does (P5, feature 01). Legacy stored a *bitfield* per word -- warn|mute|kick|ban|report
-- could all be set at once, and its runner then resolved them into a precedence order buried in a hundred lines
-- of `cleanup`. One action per policy here: "warn and also mute and also kick" is not a coherent punishment,
-- and the coherent version of it already exists as the warn ladder (P2), which escalates on the *count* rather
-- than on one message.
--
-- REPORT files a report card instead of punishing, which is feature 30 ("report instead of delete"). It is only
-- offered when the native rule is set to alert rather than block: a blocked message never existed as far as
-- Discord is concerned, so there is nothing for the card to link to.
CREATE TYPE automoderator_banword_action AS ENUM (
  'WARN',
  'MUTE',
  'KICK',
  'BAN',
...
-- The response half of the AutoMod hybrid: Discord matches, we react. See the port doc's "The AutoMod hybrid".
--
-- **Keyed on `(rule_id, keyword)`, with a NULL keyword meaning "any hit on this rule".** The port doc originally
-- specified `matched_keyword` alone, which is right for a keyword rule and cannot express the other two kinds
-- Discord has: a *preset* rule (Profanity / Sexual Content / Slurs) matches against a word list we are not able
-- to enumerate, so there is no keyword for a policy to name, and a *regex* rule returns the pattern, which
-- nobody wants to attach policies to one at a time. A nullable keyword covers all three and keeps the per-word
-- model where it is actually useful. Keyword-level wins over rule-level when both match -- the specific rule
-- beats the blanket one, which is the only ordering that lets a guild say "this whole list warns, except that
-- one word bans".
--
-- `rule_id` is a Discord id we do not own and cannot constrain: a rule deleted on Discord's side leaves its
-- policies behind, inert, and the dashboard renders them as orphaned rather than hiding them. That is the
-- deliberate reading -- silently deleting somebody's configured punishments because a rule was momentarily
-- unreadable is the worse failure, and the port never writes to Discord's AutoMod (see the P9 section of the
-- port doc, which no longer migrates legacy banned words into native rules at all).
CREATE TABLE automoderator_banword_policies (
  id               INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  guild_id         TEXT NOT NULL,
  rule_id          TEXT NOT NULL,
  -- NULL is a real value here, not "unset" -- hence the NULLS NOT DISTINCT on the unique constraint below,
  -- which is what stops a guild ending up with two competing rule-level policies for one rule.
  keyword          TEXT,
  action_type      automoderator_banword_action NOT NULL,
  -- Seconds, matching `automoderator_warn_punishments.duration_seconds` (and NOT legacy's unbounded
  -- milliseconds in a BIGINT). NULL is a permanent ban, and is the only reading for the three actions that
  -- carry no duration at all.
  duration_seconds INTEGER,

  CONSTRAINT automoderator_banword_policies_key
    UNIQUE NULLS NOT DISTINCT (guild_id, rule_id, keyword),

  -- The same split as `automoderator_warn_punishments_duration_check`, for the same reason: a MUTE is a native
...
-- The lookup every `AUTO_MODERATION_ACTION_EXECUTION` does: one rule's policies, keyword-level and rule-level
-- together, so the bot resolves precedence in memory off a single small read rather than issuing two queries
-- per event.
...
CREATE TABLE appeals_settings (
  ...
  -- immediately; a forum channel gets one post per appeal. Either way: exactly one embed, edited in place for
  -- the life of the appeal (decision 13).
  mod_channel_id        TEXT NOT NULL,
  -- How long after a decision before the same user may appeal the same punishment again. `0` is a real value
  -- meaning "resubmit as soon as a decision lands".
  ...
  CONSTRAINT appeals_settings_cooldown_days_check CHECK (cooldown_days BETWEEN 0 AND 365),
  CONSTRAINT appeals_settings_max_appeals_check CHECK (max_appeals IS NULL OR max_appeals BETWEEN 1 AND 10)
);

-- The guild's appeal questionnaire. Seeded from `DEFAULT_APPEAL_QUESTIONS` (@chatsift/core) the first time a
-- guild saves its config, so every guild has real rows from day one and P7's editor is plain CRUD over them
-- rather than a migration that has to invent history for guilds configured before it shipped.
--
-- No `max_length` column: an answer is an embed field value, which Discord caps at 1024 for everyone
-- (`APPEAL_ANSWER_MAX_LENGTH`), and a per-question limit a guild could raise would configure an appeal the
-- bot cannot post.
CREATE TABLE appeal_questions (
  id         INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  guild_id   TEXT NOT NULL,
  -- Display order, 0-based and dense. Not unique per guild: P7 reorders by rewriting the whole set, and a
  -- unique constraint would make a swap need either a deferred constraint or a park-at-negative dance for no
  -- gain -- the set is capped at `APPEAL_QUESTION_MAX_COUNT` and always read in full.
...
-- Read in full, ordered, on every appeal form render and every submit.
...
-- decision_reason (see docs/roadmap/09-appeals.md §5 -- that serializer is the single choke point, and a leak
-- here is a one-line mistake nothing else in the stack would catch).
CREATE TABLE appeals (
  ...
  -- Silence is a property of a denial and of nothing else -- an approval or a withdrawal the appellant cannot
  -- see would be a bug that reads as a feature.
  CONSTRAINT appeals_silent_check CHECK (NOT silent OR status = 'DENIED'),
  -- Enumerated exhaustively with an `ELSE false`, deliberately: a `CASE` with no `ELSE` yields NULL for a
  -- value it doesn't list, and a CHECK passes on NULL -- so the "safe" version of this constraint would
  -- silently accept every row a future `appeal_status` value could produce, which is exactly the class of bug
  -- the trigger-ladder CHECK shipped with.
...
-- One open appeal per (guild, user, punishment kind). The submit path checks the cooldown and `max_appeals`
-- itself and reports properly on both, but this is what makes a double-submit race impossible rather than
...
-- The appellant's own "track your appeals" view, and the cooldown/ceiling lookup at submit.
CREATE INDEX appeals_user_id_id_idx ON appeals (user_id, id DESC);

-- Answers, each carrying a snapshot of the prompt it answered (decision 7) -- editing a question later never
-- rewrites history, which is also what lets P7's questionnaire editor be additive rather than a migration.
...
CREATE TABLE appeal_ban_checks (
  ...
  -- Read by P8's pattern matching. Comes from the same probe call that answered `banned`, at no extra cost.
  ban_reason TEXT,
...
-- Users a guild has manually marked as unable to appeal (decision 8). The automatic half is
-- `unappealable_patterns` below.
CREATE TABLE unappealable_users (
  guild_id      TEXT NOT NULL,
  user_id       TEXT NOT NULL,
  -- Moderator-facing only. The appellant is told they cannot appeal, never why.
  reason        TEXT,
  created_by_id TEXT NOT NULL,
  created_at    TIMESTAMPTZ NOT NULL DEFAULT now(),
...
-- P8: matched against the ban reason the probe returns, to mark a ban unappealable without anyone having to
-- list the user by hand.
--
-- **A plain case-insensitive substring, not a regular expression.** These are written by guild moderators and
-- evaluated in the API process on a public, unauthenticated-by-design path; a user-supplied regex there is a
-- ReDoS surface, and "the reason contains `ban evasion`" is what every guild actually means anyway.
...
-- Per-appellant state, keyed by user rather than by (user, guild): DM reachability and the OAuth grant are
-- both properties of the person, not of any one appeal. **Nothing here is ever exposed to a guild's
-- moderators** -- the mod-side embed is told only that delivery looks unlikely, never anything else about the
-- account.
CREATE TABLE appeal_user_state (
  user_id             TEXT PRIMARY KEY,
  -- Whether a DM has actually been delivered to this user (decision 10 / §6: a user install of the Appeals
  -- application is what makes this possible at all with no shared guild). NULL = never attempted. Both
  -- surfaces warn *before* a decision is made when this is false, since a decision that cannot be delivered
  ...
  -- appellant authorized, so the grant has to survive that gap -- and deleted once every appeal of theirs is
  -- terminal and the re-add window has passed. NULL when they never granted `guilds.join`.
  refresh_token       TEXT,
  updated_at          TIMESTAMPTZ NOT NULL DEFAULT now()
);

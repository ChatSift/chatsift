...
export async function postToQueue({
	...
	const buttons: APIButtonComponent[] = [
		...
		// #366. Messages posted before this button existed keep their three-button row for life -- a queue
		// message's components are only rewritten when someone acts on it, so this reached new submissions
		// rather than retrofitting the backlog. Nothing was lost: the dashboard's own toggle covers everything

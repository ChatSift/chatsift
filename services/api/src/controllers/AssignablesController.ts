...
export class AssignablesController {
	public constructor(@inject(kSql) public readonly sql: Sql<{}>, public readonly prompts: PromptsController) {}

	public get(roleId: Snowflake): Promise<SelfAssignableRole | undefined> {
		return this.sql<
			[SelfAssignableRole?]
		>`SELECT * FROM self_assignable_roles WHERE role_id = ${roleId} ORDER BY id`.then((rows) => rows[0]);
	}
	...
	public delete(roleId: Snowflake): Promise<SelfAssignableRole | undefined> {
		return this.sql<
			[SelfAssignableRole?]
		>`DELETE FROM self_assignable_roles WHERE role_id = ${roleId} RETURNING *`.then((rows) => rows[0]);
	}

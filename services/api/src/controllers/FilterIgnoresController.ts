...
export class FilterIgnoresController {
	...
	public get(channelId: Snowflake): Promise<FilterIgnore | undefined> {
		return this.sql<[FilterIgnore?]>`SELECT * FROM filter_ignores WHERE channel_id = ${channelId}`.then(
			(rows) => rows[0],
		);
	}
	...
	public delete(channelId: Snowflake): Promise<FilterIgnore | undefined> {
		return this.sql<[FilterIgnore?]>`DELETE FROM filter_ignores WHERE channel_id = ${channelId} RETURNING *`.then(
			(rows) => rows[0],
		);
	}

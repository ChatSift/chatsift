...
export class PromptsController {
	...
	public async get(gid: Snowflake, pid: number): Promise<PromptData | undefined> {
		...
		>`SELECT * FROM self_assignable_roles_prompts WHERE guild_id = ${gid} AND prompt_id = ${pid}`;

		if (prompt) {
			const roles = await this.sql<
				SelfAssignableRole[]
			>`SELECT * FROM self_assignable_roles WHERE prompt_id = ${pid} ORDER BY id`;
			return { ...prompt, roles };
		}
	}
	...
	public async getByMessage(id: Snowflake): Promise<PromptData | undefined> {
		const [prompt] = await this.sql<
			[SelfAssignableRolePrompt?]
		>`SELECT * FROM self_assignable_roles_prompts WHERE message_id = ${id}`;

		if (prompt) {
			const roles = await this.sql<
				SelfAssignableRole[]
			>`SELECT * FROM self_assignable_roles WHERE prompt_id = ${prompt.prompt_id} ORDER BY id`;
			return { ...prompt, roles };
		}
	}
	...
	public async getAll(guild: Snowflake): Promise<PromptData[]> {
		const prompts = await this.sql<
			SelfAssignableRolePrompt[]
		>`SELECT * FROM self_assignable_roles_prompts WHERE guild_id = ${guild}`;
		const roles = await this.sql<
			SelfAssignableRole[]
		>`SELECT * FROM self_assignable_roles WHERE guild_id = ${guild} ORDER BY id`;
	...
	public add(data: Partial<Omit<SelfAssignableRolePrompt, 'prompt_id'>>): Promise<SelfAssignableRolePrompt> {
		return this.sql<[SelfAssignableRolePrompt]>`INSERT INTO self_assignable_roles_prompts ${this.sql(
			data,
	...
	public update(data: {
		...
		return this.sql<[SelfAssignableRolePrompt?]>`
	...
	public delete(gid: Snowflake, pid: number): Promise<SelfAssignableRolePrompt | undefined> {
		return this.sql<
			[SelfAssignableRolePrompt?]
		>`DELETE FROM self_assignable_roles_prompts WHERE guild_id = ${gid} AND prompt_id = ${pid} RETURNING *`.then(
			(rows) => rows[0],
		);
	}

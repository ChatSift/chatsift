...
export class UrlsAllowlistController {
	...
	public get(guildId: Snowflake, domain: string): Promise<AllowedUrl | undefined> {
		return this.sql<[AllowedUrl?]>`SELECT * FROM allowed_urls WHERE guild_id = ${guildId} AND domain = ${domain}`.then(
			(rows) => rows[0],
		);
	}
	...
	public async add(guildId: Snowflake, domain: string): Promise<AllowedUrl | undefined> {
		...
		return this.sql<
			[AllowedUrl]
		>`INSERT INTO allowed_urls (guild_id, domain) VALUES (${guildId}, ${domain}) RETURNING *`.then((rows) => rows[0]);
	}

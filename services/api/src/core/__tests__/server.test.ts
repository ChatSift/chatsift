...
// (via `context.js`) eagerly parses `process.env` against its full schema at module-load time -- hence
// `stubTestEnv()`, which is required just to let the module load (see its own doc comment).
// `publishRealtimeInvalidate` itself is stubbed down to just recording calls -- `mountRoute`'s

...
export default class extends Route {
	...
	private async createWarnCase(
		sql: Sql<{}>,
		data: CaseData & { guild_id: Snowflake },
		cs: Case,
    ...
    `;
      ...
      `.then((rows) =>
				rows.reduce<ResolvedCases>((acc, cs) => {
					if (cs) {
            ...
            `;
						...
						triggeredCs = await this.createCase(
							sql,
							{
								...baseData,
		...
		const warnCaseData = {
			...(cs as Omit<Case, 'action_type'> & { action_type: CaseAction.warn; extra?: WarnCaseExtras }),
		};
		...
		if (triggeredCs || updatedCs) {
			...
			)[(triggeredCs ?? updatedCs)!.action_type as CaseAction.kick | CaseAction.mute | CaseAction.ban];

			warnCaseData.extra =
				triggered === WarnPunishmentAction.kick ? { triggered } : { triggered, duration, extendedBy };
		}
	...
	private async createCase(sql: Sql<{}>, data: CaseData & { guild_id: Snowflake }, settings?: GuildSettings) {
		...
		if (data.execute) {
            ...
            `;
						...
						const guildRoles = new Map(
							await this.rest
								.get<APIRole[]>(`/guilds/${data.guild_id}/roles`)
								.catch(() => [] as APIRole[])
            ...
            `;
						...
						const roles = await sql<UnmuteRole[]>`SELECT role_id FROM unmute_roles WHERE case_id = ${muteCase.id}`.then(
							(rows) => rows.map((role) => role.role_id),
						);
		...
		if (data.reference_id) {
			const [refCs] = await sql<
				[Case?]
			>`SELECT * FROM cases WHERE guild_id = ${data.guild_id} AND case_id = ${data.reference_id}`;

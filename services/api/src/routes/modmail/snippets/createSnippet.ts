...
export default defineRoute({
	...
	async handler(req): Promise<CreateSnippetResult> {
		...
		// single outer try/catch covering the name check through the DB insert means every failure path (duplicate
		// name, a Discord-rejected command name, a DB error) releases the claim, so a correctable mistake doesn't
		...
		try {
			...
			try {
				// `req.tokens` is unset for a grant-authed request (the grant fast path never populates it) --
				// fall back to the grant's own `sub` (the Discord user who ran the command that minted it) for
				// the actor id in that case.
				const createdById = req.grant?.sub ?? req.tokens!.access.sub;

...
export default class GetGhostCallbackRoute extends Route {
  ...
  public async handle(req: Request, res: Response, next: NextHandler) {
    ...
    const token = jwt.sign({
      ...
      exp: iat + (5 * 60),
      aud: '/canary/admin'
    }, Buffer.from(GHOST_SECRET, 'hex'), { algorithm: 'HS256', keyid: GHOST_ID });
    ...
    const signinResponse = await fetch(`${this.config.ghostDomain}/ghost/api/canary/admin/members/${user.id}/signin_urls`, {
      headers: {
        'Authorization': `Ghost ${token}`,
        'Content-type': 'application/json'
      }

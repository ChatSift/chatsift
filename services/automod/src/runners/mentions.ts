...
export class MentionsRunner {
	...
	public constructor(@inject(kLogger) public readonly logger: Logger, @inject(kRedis) public readonly redis: Redis) {}
	...
	private async runMentionSpam(message: APIMessage, amount: number, time: number): Promise<Snowflake[]> {
		...
		const { messages, mentions } = data.reduce<{ messages: string[]; mentions: string[] }>(
			(acc, entry) => {

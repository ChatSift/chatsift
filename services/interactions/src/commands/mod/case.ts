  ...
  public async exec(interaction: APIGuildInteraction, args: ArgumentsOf<typeof CaseCommand>) {
    switch (Object.keys(args)[0] as keyof typeof args) {
      ...
      case 'delete': {
        ...
        const cs = await this.rest.get<ApiGetGuildsCaseResult>(`/guilds/${interaction.guild_id}/cases/${caseId}`).catch(() => null);
        if (!cs) {
        ...
        if (cs.ref_id) {
          refCs = await this.rest.get<ApiGetGuildsCaseResult>(`/guilds/${interaction.guild_id}/cases/${cs.ref_id}`).catch(() => null);
        }

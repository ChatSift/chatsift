  ...
  public async exec(interaction: APIGuildInteraction, args: ArgumentsOf<typeof PurgeCommand>) {
    ...
    if (!toPurge.length) {
      return send(interaction, {
        content: 'There were no messages to purge. Keep in mind Discord only supports purging messages that aren\'t older than 14 days.'
      });
    }
    ...
    for (let i = 0; i < loops; i++) {
      ...
      if (messages.length === 1) {
        promises.push(
          this.discordRest.delete<never>(
            Routes.channelMessage(channelId, messages[0]!), {
              reason
            }
          )
        );

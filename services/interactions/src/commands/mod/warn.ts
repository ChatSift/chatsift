...
import { CaseManager, dmUser, PermissionsChecker, UserPerms } from '@automoderator/util';
import { Rest } from '@cordis/rest';
import { APIGuild, APIGuildInteraction, InteractionResponseType, Routes } from 'discord-api-types/v9';
...
export default class implements Command {
	...
	public async exec(interaction: APIGuildInteraction, args: ArgumentsOf<typeof WarnCommand>) {
		...
		await dmUser(
			member.user.id,
			`Hello! You have been warned in ${guild.name}.${reason ? `\n\nReason: ${reason}` : ''}`,
		);

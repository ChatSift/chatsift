...
export default class implements Component {
  ...
  private async _filter(guildId: string, userId: Snowflake, embed?: APIEmbed) {
    ...
    const allWords = await this
      .sql<BannedWord[]>`SELECT * FROM banned_words WHERE guild_id = ${guildId}`
      .then(
        words => new Map(
    ...
    for (const word of foundWords) {
      ...
      if (caseData.length) {
        const cases = await this.rest.post<ApiPostGuildsCasesResult, ApiPostGuildsCasesBody>(`/guilds/${guildId}/cases`, caseData);
    ...
    await this.discordRest.patch<unknown, RESTPatchAPIGuildMemberJSONBody>(Routes.guildMember(guildId, member.user!.id), {
      data: {
        nick: name
      }
    });

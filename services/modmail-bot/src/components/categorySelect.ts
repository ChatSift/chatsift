...
export default class CategorySelectComponent implements ComponentHandler {
	...
	public async handle(interaction: APIMessageComponentInteraction, _state: never, logger: Logger): Promise<void> {
		...
		await withGuildUserLock(guildId, user.id, async () => {
			...
			// Everything from the per-category limit check onwards is shared with `createTicket.ts`'s own
			// no-picker paths (zero or one category) — see `lib/panelTicket.ts`.
			await createPanelTicket({ category, guildId, guildSettings, interaction, logger, member, panel });
		});
	}
}

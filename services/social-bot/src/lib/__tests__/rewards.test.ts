...
test('non-clean rewards accumulate', () => {
	...
	const diff = computeRewardRoleDiff({ heldRoleIds: [], level: 5, rewards });
...
test('only the highest clean tier is held', () => {
	const diff = computeRewardRoleDiff({ heldRoleIds: [], level: 7, rewards: TIERS });
...
test('a promotion swaps the previous tier out', () => {
	const diff = computeRewardRoleDiff({ heldRoleIds: [REGULAR], level: 10, rewards: TIERS });
...
test('a tier the member no longer qualifies for is stripped', () => {
	...
	const diff = computeRewardRoleDiff({ heldRoleIds: [VETERAN], level: 5, rewards: TIERS });
...
test('nothing happens when the member already has exactly the right roles', () => {
	...
	const diff = computeRewardRoleDiff({ heldRoleIds: [REGULAR, HELPER], level: 5, rewards });
...
test('roles that are not rewards are never touched', () => {
	...
	const diff = computeRewardRoleDiff({ heldRoleIds: [UNRELATED, REGULAR], level: 5, rewards: TIERS });
...
test('a level 0 member earns nothing and keeps nothing', () => {
	const diff = computeRewardRoleDiff({ heldRoleIds: [NEWCOMER], level: 0, rewards: TIERS });
...
test('clean and non-clean rewards coexist', () => {
	...
	const diff = computeRewardRoleDiff({ heldRoleIds: [NEWCOMER], level: 5, rewards });
...
test('a multi-level jump grants every tier it crossed at once', () => {
	...
	const rewards = [...TIERS, { clean: false, level: 3, roleId: HELPER }];
	const diff = computeRewardRoleDiff({ heldRoleIds: [], level: 10, rewards });
...
test('the patched role array never duplicates a role', () => {
	...
	const diff = computeRewardRoleDiff({ heldRoleIds: [REGULAR], level: 5, rewards: TIERS });
